...
| Function | Purpose |
|----------|---------|
| `getImage(media, view, color?)` | Best single image for a view, with color fallback |
| `getImageWithFallback(media, views[], color?)` | Try views in priority order, return first match |
| `resolveImage(media, views[], stemExists, color?)` | Like `getImageWithFallback` but also checks filesystem via predicate |
| `getCarouselImages(media, color?)` | All photo views for a color (excludes SVG shapes) |
| `getImageForColor(media, color, view)` | Image for a specific color + view |
...
Per-category image configuration. Managed by `config/image-manager.pyw`.
...
The **only** place in the codebase that constructs image URLs. Two functions, nothing else.
