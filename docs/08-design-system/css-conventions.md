...
### How to generate a new fluid variable

If a `--ft-A-B`, `--ftm-A-B`, or `--fm-A-B` doesn't exist yet, create it:

| Family | Viewport range | Base rem | Denominator |
|--------|---------------|----------|-------------|
| `--ft-A-B` | 1150 → 600 | 11px | 550 |
| `--ftm-A-B` | 825 → 600 | 11px | 225 |
| `--fm-A-B` | 600 → 400 | 10px | 200 |

**Formula:**
```
slope_vw     = (A − B) / denominator
intercept_px = B − (slope_vw × minVW)
intercept_rem = intercept_px / base

Result: clamp(var(--font-size-Bpx), {intercept_rem}rem + {slope_vw × 100}vw, var(--font-size-Apx))
```

**Worked example — `--fm-9-8` (600→400, base 10px):**
```
slope     = (9 − 8) / 200 = 0.005     → 0.5vw
intercept = 8 − (0.005 × 400) = 6px   → 6 / 10 = 0.6rem
Result:   clamp(var(--font-size-8px), 0.6rem + 0.5vw, var(--font-size-9px))
```

New variables go in the matching `:root` block in `global.css`, sorted descending by A value.

### Rules

1. **NEVER use raw `px` or `em` for font sizes.** Use `var(--font-size-Xpx)` (static) or a fluid variable (`--ft-*`, `--ftm-*`, `--fm-*`).
2. **NEVER use raw `rem` for font sizes or icon/SVG sizing.** Use `var(--font-size-Xpx)` — SVGs use `height: 1em` with `font-size` on the container. Raw `rem` is fine for non-font layout values (padding, margins, widths, gaps).
3. **NEVER hardcode a `clamp()` inline.** Always define a named variable in `global.css` and reference it.
4. **Pick the right tool:** Use static vars (`--font-size-Xpx`) for fixed sizes. Use fluid vars (`--ft-*`, `--ftm-*`, `--fm-*`) when a size should scale smoothly between breakpoints. Both are valid.
5. **When converting HBS CSS:** if you see `1.27273rem`, that's `14 / 11` -> use `var(--font-size-14px)`.
6. **Quick conversion:** multiply the rem value by 11 to get the pixel intent. Example: `2.54545rem * 11 = 28px` -> `var(--font-size-28px)`.
7. **If a `--font-size-Xpx` doesn't exist** for your target, add it to all three `:root` blocks (`X÷11`, `X÷10`, `X÷9`).
...
## 7. Vertical Spacing System (`--section-gap`)

### The variable

```css
:root {
  --section-gap: clamp(2.5rem, 2rem + 1.5vw, 4rem);
}
```

One variable controls all vertical spacing between major sections on content pages.

| Viewport | Resolved value (at 11px base) |
|----------|-------------------------------|
| 400px | ~27.5px (floor) |
| 600px | ~30.5px |
| 768px | ~33.5px |
...
### Where it's used (home page)

| Selector | Property | Previous value |
|----------|----------|---------------|
| `.home-page-body` | `margin: var(--section-gap) auto` | `2rem auto` |
| `.home-page-body.home-news` | `margin: var(--section-gap) auto` | `5rem auto 2rem auto` |
| `.home-page-body--first` | `margin-top: var(--section-gap)` | `2rem` |
| `.home-ad` | `margin: var(--section-gap) 0` | `42px 0 0` (was asymmetric) |
| `.section-divider` | `margin: var(--section-gap) auto 2.25rem auto` | `2.75rem auto 2.25rem auto` |
| `.sub-section-divider-container` | `margin: var(--section-gap) auto 2rem auto` | `4rem auto 2rem auto` |

### Rules for new pages

1. **Use `var(--section-gap)` for spacing between major sections.** Never hardcode `2rem`, `4rem`, `42px`, etc. for section-to-section gaps.
2. **Ads must be symmetric.** `margin: var(--section-gap) 0` — equal above and below. No `margin: Xpx 0 0` (asymmetric gap is visually jarring).
3. **Bottom margins on dividers stay fixed.** Only the top margin uses `--section-gap`. The bottom margin (`2.25rem` for SectionDivider, `2rem` for SubSectionDivider) is the gap to the content immediately below — tighter by design.
4. **One knob to tune.** If the page feels too loose or too tight, adjust `--section-gap` in `global.css` — all sections scale together.
5. **Grid BFC boundary.** `.home-rail-anchor` uses `display: grid`, which prevents margin collapse across section boundaries. `--section-gap` ensures consistent spacing regardless of margin collapse behavior.
...
## 10. CSS Traps (learned during build)
