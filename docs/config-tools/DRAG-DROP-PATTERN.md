# Drag-and-Drop Pattern — Config Tools

All three config tools share a common drag-and-drop architecture built on
...
### navbar-manager.py — Brands Tab

| Source | Target | Action |
|--------|--------|--------|
| "All Brands" pool | Category column | Add category to brand's `navbar` list |
| Category column | "All Brands" pool | Remove category from brand's `navbar` list |
| Category column | Different category column | Move brand between categories |

- Brands can belong to **multiple categories** (navbar list is an array)
- Drag between category columns adds the target category
- Delete key removes from current category

### navbar-manager.py — Guides Tab
...
### hub-tools-manager.py — Index Tab
